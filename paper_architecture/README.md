...
small transverse-field Ising example through it, with exact gradient
normalization, additional measurement support, and reuse on a fixed reference.
It distinguishes odd-Y filtering in real-state ADAPT from the whole conserving
excitation operators used by molecular A-CASE. The feature table
...
- `references.bib` — bibliography. Shared entries follow the companion
  manuscripts, with three documented metadata corrections: the companion
  preprint uses an `eprint` field to avoid a duplicated identifier; Somaroo,
  Cory, and Havel use the publisher's DOI
  [10.1016/S0375-9601(98)00010-3](https://doi.org/10.1016/S0375-9601(98)00010-3);
  and Hrdina, Návrat, and Vašík include
  [10.1007/s11128-022-03648-w](https://doi.org/10.1007/s11128-022-03648-w).
  The published snapshot in `paper_a_case_subspaces/` remains unchanged.
...
|---|---|
| Operator-centric section and its worked example | Package interfaces (`clifford_qc/pauli_kernel.py`, `clifford_qc/measurement/bank.py`, `clifford_qc/measurement/cache.py`, `clifford_qc/algorithms/pools.py`) and the companion preprint; the example is algebra, not a measurement |
