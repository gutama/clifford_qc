...
\title{Making a simulation framework falsifiable: one operator\\
representation, labelled evidence, and preregistered gates}
...
commitments carry the design.  First, one sparse Pauli-word operator type
represents states, gates, observables, channels, Jordan--Wigner generators and
fermionic ladder operators alike, so there is no conversion boundary between
circuit and operator descriptions; the package is \cqcModules{} modules and
\cqcLines{} physical lines against \cqcTestLines{} lines of tests.  Second,
every number carries the kind of evidence it is, from the measurement layer
through the solvers into committed records, and benchmark gates compare against
that label.  We report what the second commitment produced.  A dyadic
...
\cqcSCSettings{} settings outright and prices nothing.  The accuracy-matched
block size is a property of the card and the estimator rather than of the
Hamiltonian, and an H$_4$ bank is never priced at all because its own subspace
...
claim is that two architectural commitments --- one representation with no
conversion boundary, and an evidence label attached to every number --- make a
...
\section{One representation, and the seams around it}
\label{sec:architecture}

\subsection{The kernel}

The package has one operator type.  A multivector \texttt{MV} is a sparse map
from packed Pauli-word codes to complex coefficients, two bits per qubit, with
multiplication implemented as exclusive-or, and-mask and population count
modulo four.  States, gates, observables, Kraus channels, Jordan--Wigner
Clifford generators and fermionic creation and annihilation operators are all
this one type.  The consequence that matters architecturally is negative: there
is no conversion boundary between ``circuit land'' and ``operator land'', so no
layer above the kernel has to own a translation, and no benchmark has to
declare which side of a translation it measured.
...
\begin{figure*}
  ...
  higher one in.  Dashed arrows are genuine upward runtime edges deferred to
  call time.  The deferral buys initialization order, not independence, and
...
The vocabulary itself is small: a quantity is exact, asymptotic, finite-sample,
heuristic, or unlabelled.  Its purpose is not taxonomy but prevention.  An
...
sub-object; and \cqcRecordsUnlabelled{} carry no record-level evidence field at
all.  A further \cqcSeriesFiles{} series files --- line-delimited JSON, CSV and
Markdown summaries --- carry no record-level field by construction.

This is a real gap and it is worth stating precisely.  The evidence vocabulary
is enforced in code, where one selection module defines it and the measurement
and subspace layers carry it in their return types, and it is enforced in the
newer records, whose producers write it at the top level.  It is not enforced by a schema over the
...
\cqcGates{} gates read them, of which \cqcGatesInCI{} run in continuous
integration, each as its own named step that executes even after an earlier one
fails, so that a drift suite reports every symptom rather than the first.
...
\begin{table}
  ...
  \label{tab:gates}
  \begin{ruledtabular}
  \begin{tabular}{lrp{0.52\columnwidth}}
  Class & Gates & Asserts \\
  \colrule
...
\subsection{The accuracy-matched block size belongs to the card, not to the
Hamiltonian}
...
and the record reports that status rather than collapsing it to an argmin.  Second, the mapping axis is not small: the spread
between the cheapest and dearest fermionic encoding at a fixed rung reaches
\cqcMappingSpreadKOne{} on the same instance.  Third, the sparse-connectivity
...
Hamiltonian's non-identity Hilbert--Schmidt norm and carry a bias of
\cqcRFourACSBias{}~mHa, an order of magnitude past the admissible budget.  The
...
manuscript checker rejects a bare decimal or a four-digit integer typed into the
body.  Each generated table fragment carries content hashes of its source
records and of its generator, which the checker re-derives, so a record
regenerated without rerunning the generator fails the build rather than
typesetting a stale number.  Figures are bound the same way through a manifest
...
one sparse Pauli-word operator type with no conversion boundary, an evidence
label carried from the measurement layer into committed records, and a
