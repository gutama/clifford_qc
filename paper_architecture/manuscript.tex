...
Two screens returned no result: the value-rebuilt mapping comparison found no
structural separation and stayed indeterminate after refinement, while the
preregistered contextual-restriction screen admitted none of its
\cqcRFourARungs{} rungs and stopped before drawing a sample.  We also report
where the contract is not yet
...
\begin{enumerate}
  ...
  it describes.  Most import-time dependencies point downward; the
  module-scope chemistry-to-pool exception and the upward edges deferred to
  call time are named individually.
...
distinct \texttt{PauliSum} container for Hamiltonians and
...
chemistry model also imports the algorithm pool at module scope, so the
import-time graph itself is not strictly layered.  Separately, the
...
defines the selection vocabulary, but measurement and subspace result types
carry their evidence labels as strings and their constructors do not validate
enum membership.  Those typed numerical paths therefore follow a convention;
they do not enforce the vocabulary.  Newer records preserve evidence at the
top level, but the broader record-label vocabulary is not normalized; it is not enforced by a schema over the
record set, so nothing today rejects a new record that omits the field.  The
...
replicas and cost runner-hours.
\cqcGatesAbsent{} exist in the repository but in no workflow at all.  The short
automatic gates are individual named steps and use continuation guards.
Structural and sampled gates are separately named matrix jobs whose matrices
disable fail-fast.  Thus a drift suite reports every symptom rather than the
first within each execution class.
...
word-universe ceiling several times over, at \cqcRFourAFullWords{} words.  The
  ...
  \cqcRFourARemovedFraction{} of the Hamiltonian's non-identity
  Hilbert--Schmidt norm; there the CS-QSE arm carries a bias of
  \cqcRFourACSBias{}~mHa and the CS-A-CASE arm carries
...
regeneration catches a value edited into a fragment by hand.  A figure manifest
binds each committed vector asset to its generator and input digests and also
records the asset's own checksum, so a replaced PDF fails the gate.  The gate
does not compare a scratch render byte for byte because plotting and font builds
need not serialize the same paths identically.
...
own bias exceeds the target, a value-rebuilt mapping screen and a
preregistered contextual screen that returned no result, a reformulation
retired against its own falsifier, and a coverage census that
