"""Dense matrix conversion and eigensolvers for validation and interoperability.

The native representation in :mod:`clifford_qc` is the sparse Pauli/Clifford
multivector.  Functions in this module deliberately materialize ``2^n x 2^n``
arrays and therefore cost ``O(4^n)`` memory.  Keep them for small-system
oracles, diagnostics that genuinely need a spectrum, and external matrix
bridges; use :mod:`clifford_qc.pauli_action` for the scaling statevector path.

``clifford_qc.matrix`` re-exports this API for backward compatibility.
"""
...
def exact_ground(H: MV):
    """Small-system dense ground-state oracle."""
    import numpy as np
