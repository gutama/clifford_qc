...
*Generated by `benchmarks/summarize_molecular.py` from `molecular/results/results_summary.json`. Do not edit by hand — regenerate. Consistency is gated by `benchmarks/check_molecular.py`.*
...
The uncorrelated column is $N_e/(2n_{\text{orb}})$, the closed-shell reference value. The difference diagnoses changes in orbital double occupancy; equality alone does not prove the state is uncorrelated.

$\langle S^2 \rangle$ checks spin contamination. Conserving $S_z$ does not enforce a singlet: the selected determinant subspace need not preserve total spin. Nonzero values, especially stretched water's, must be assessed before comparing states of different multiplicity.

### 2.1 Response limitations

The site-0 $S_z$ Lehmann response is computed only within the selected Ritz space. Negligible projected weight does not establish a vanishing physical susceptibility. The historical diagnostics below are preserved verbatim; their claim that the excitation basis preserves singlets is not guaranteed by $S_z$ conservation and is superseded by this caveat.
...
A physical response claim would require checking the relevant excited spin sectors and convergence of their transition weights. This suite does not perform that validation.
...
The adaptive arm uses fewer selected directions, by retaining a smaller selected block; screening can still build pairs for rejected candidates.
...
```bash
python -m molecular.run --output-directory molecular/results           # regenerate the records
...
Records, FCIDUMPs and per-molecule JSON are in [`molecular/results/`](results/).
